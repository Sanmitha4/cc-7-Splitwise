  ...
  const showFriendForm = async () => {
    try {
      while (!friendFormData.name) {
        const input =
          (await ask("Enter friend's name:", { validator: nameValidator })) ??
          "";
      ...
      while (!friendFormData.email) {
        const input =
          (await ask("Enter friend's email:", { validator: emailValidator })) ??
          "";
      ...
      while (!friendFormData.phone) {
        const input =
          (await ask("Enter friend's phone:", { validator: phoneValidator })) ??
          "";
      ...
      friendFormData.openingBalance =
        (await ask("Enter opening balance", { validator: numberValidator })) ??
        "0";
...
const updateFriend = async () => {
  const query = await ask(
    "Search for the friend to update (Name/Email/Phone):",
  );
  if (!query) return;
  ...
  const indexInput = await ask(
    "Enter the Index number of the friend to update (or Enter to cancel):",
    {
      validator: (val) => {
        if (val === "") return true;
        const num = parseInt(val);
        return (
          (!isNaN(num) && num > 0 && num <= queryResult.match) ||
          "Please enter a valid index from the table."
        );
      },
    },
  );
  ...
  const nameInput =
    (await ask(`New name [${selectedFriend.name}]:`)) || selectedFriend.name;
  ...
  while (!emailValid) {
    const input =
      (await ask(`New email [${selectedFriend.email}]:`)) ||
    ...
    if (formatCheck !== true) {
      ...
      const otherFriend = friendsRepository
        .getAllFriends()
        .find(
          (f) =>
            f.email.toLowerCase() === input.toLowerCase() &&
  ...
  while (!phoneValid) {
    const input =
      (await ask(`New phone [${selectedFriend.phone}]:`)) ||
    ...
    if (formatCheck !== true) {
      ...
      const otherFriend = friendsRepository
        .getAllFriends()
        .find((f) => f.phone === input && f.id !== selectedFriend.id);

      if (otherFriend) {
        console.log(
          `The phone number ${input} is already registered to another friend.`,
        );
      } else {
        newPhone = input;
        phoneValid = true;
...
const removeFriend = async () => {
  const query = await ask(
    "Search for the friend to delete (Name/Email/Phone):",
  );
  if (!query) return;
  ...
  const indexInput = await ask(
    "Enter the Index number to delete (or Enter to cancel):",
    {
      validator: (val) => {
        if (val === "") return true;
        const num = parseInt(val);
  ...
  if (selectedFriend.balance !== 0) {
    ...
    const confirmBalance = await ask(
  ...
  const finalConfirm = await ask(
    `Are you absolutely sure you want to delete ${selectedFriend.name}? (yes/no):`,
  );
